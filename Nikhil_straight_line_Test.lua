...
function update()
  if not arming:is_armed() then -- reset state when disarmed
    ...
    if pwm6 and pwm6 > 1800 then    -- check if RC7 input has moved high
      ...
      elseif (stage == 3) then   -- fly in a straight line 
        local curr_loc = ahrs:get_location()
        ...
        if (start_loc and curr_loc) then
          ...
          target_vel:x(2) -- flies North at 2m/s
          if (dist_NE:x() >= line_length) then
            stage = stage + 1
          end
