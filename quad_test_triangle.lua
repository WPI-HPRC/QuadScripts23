...
function update() 
    ...
    if arming:is_armed() and vehicle:get_mode() == AUTO_MODE then
        if ahrs:healthy() then 
            ...
            local final_alt = altitude - home_alt  --this is different than defining alt in release script, should probably add additional if statements to set mode if not in auto mode
            if final_alt > 1457 then --make sure altitude units are correct (cm???)
                target_vel:z(-2)
            end

            if final_alt <= 457 then 
                for i = 0,2 do 
                    ...
                    local current_location = ahrs.get_position(); --set home location to current point and calculate distance that way
