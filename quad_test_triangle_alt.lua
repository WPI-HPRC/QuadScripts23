...
local state = 0
local tri_state = 0
local target_alt = 7 --target alt in meters for general routine 
local target_alt_drop = 3 --target alt to drop the cubes 
    ...
    if not arming:is_armed() then
        ...
        elseif (state == 2) then    
            ...
            for i = 0,2 do 
                ...
                elseif (tri_state == 2 or tri_state == 3) then
                    local home = ahrs:get_home()
                    local curr_loc = ahrs:get_location()
                    local target_vel = Vector3f()  
                    ...
                    if tri_state == 2 then
                        ...
                        gcs:send_text(0, "alt above home: " .. tostring(math.floor(-vec_from_home:z())))
                            ...
                            if (target_alt_drop + vec_from_home:z()) > 1 then
                                target_vel:z(2)
                            elseif (target_alt_drop + vec_from_home:z()) < -1 then
                                target_vel:z(-2)
                            elseif (math.abs(target_alt_drop + vec_from_home:z()) < 1) then
                                tri_state = tri_state + 1
                                --start_location = curr_loc          -- record location when starting square
                            end
                        end

                    elseif tri_state == 3 then
                        if home and curr_loc then
                            local vec_from_home = home:get_distance_NED(curr_loc)
                            gcs:send_text(0, "alt above home: " .. tostring(math.floor(-vec_from_home:z())))
